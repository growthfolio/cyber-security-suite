...
import com.codexraziel.cybersec.controllers.WiFiController;
...
public class WiFiView {
    ...
    public VBox createView(Label statusLabel) {
        ...
        cancelBtn.setGraphic(CodexIcons.STOP);
        ...
        scanStatus.setGraphic(CodexIcons.SUCCESS);
        ...
        statusCol.setCellFactory(column -> new TableCell<String[], String>() {
            ...
            protected void updateItem(String item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    setText(item);
                    if (item.contains("RED")) setStyle("-fx-text-fill: red; -fx-font-weight: bold;");
                    else if (item.contains("YELLOW")) setStyle("-fx-text-fill: orange; -fx-font-weight: bold;");
                    else if (item.contains("GREEN")) setStyle("-fx-text-fill: green; -fx-font-weight: bold;");
                    else if (item.contains("SATELLITE")) setStyle("-fx-text-fill: blue; -fx-font-weight: bold;");
                }
            }
        });
        ...
        browseWordlist.setGraphic(CodexIcons.FOLDER);
        ...
        Button startWifiAttack = new Button("Start Attack");
        startWifiAttack.setGraphic(CodexIcons.START);
        Button stopWifiAttack = new Button("Stop Attack");
        stopWifiAttack.setGraphic(CodexIcons.STOP);
        Button captureHandshake = new Button("Capture Handshake");
        captureHandshake.setGraphic(CodexIcons.WIFI);
        ...
        saveProfileBtn.setGraphic(CodexIcons.DOWNLOAD);
        Button loadProfileBtn = new Button("Load Profile");
        loadProfileBtn.setGraphic(CodexIcons.FOLDER);
        Button exportBtn = new Button("Export Results");
        exportBtn.setGraphic(CodexIcons.DOWNLOAD);
        ...
        Label discoveryLabel = new Label("Network Discovery:");
        discoveryLabel.setGraphic(CodexIcons.SEARCH);
        Label networksLabel = new Label("Available Networks:");
        networksLabel.setGraphic(CodexIcons.WIFI);
        Label configLabel = new Label("Attack Configuration:");
        configLabel.setGraphic(CodexIcons.TOOLS);
        Label controlsLabel = new Label("Attack Controls:");
        controlsLabel.setGraphic(CodexIcons.BRUTEFORCE);
        Label profileLabel = new Label("Profile & Export:");
        profileLabel.setGraphic(CodexIcons.DOWNLOAD);
        Label logLabel = new Label("Attack Log:");
        logLabel.setGraphic(CodexIcons.INFO);
