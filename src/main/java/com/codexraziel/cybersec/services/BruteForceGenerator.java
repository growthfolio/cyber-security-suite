...
public class BruteForceGenerator {
    ...
    public CompletableFuture<BruteForceStats> startBruteForce(BruteForceConfig config, String target, 
        ...
        return CompletableFuture.supplyAsync(() -> {
            if (isRunning.get()) {
                logCallback.accept("[ERROR] Brute force already running!");
                return null;
            }
    ...
    private BruteForceStats executeBruteForce(BruteForceConfig config, String target,
        ...
        logCallback.accept("[CONFIG] Configuration:");
        ...
        logCallback.accept("[INFO] Charset: " + charset + " (" + charset.length() + " characters)");
        
        long totalCombinations = calculateTotalCombinations(charset.length(), config.minLength, config.maxLength);
        logCallback.accept("[STATS] Total combinations: " + totalCombinations);
        
        if (totalCombinations > config.maxAttempts) {
            logCallback.accept("[WARN] Limited to " + config.maxAttempts + " attempts");
        ...
        logCallback.accept("[START] Starting brute force attack...");
        
        // Generate and test passwords
        for (int length = config.minLength; length <= config.maxLength && isRunning.get(); length++) {
            logCallback.accept("[STRAIGHT_RULER] Testing passwords of length " + length);
        ...
        if (stats.found) {
            logCallback.accept("[CELEBRATION] SUCCESS! Password found: " + stats.foundPassword);
            logCallback.accept("[TIME] Time taken: " + (stats.elapsedTimeMs / 1000.0) + " seconds");
            logCallback.accept("[STATS] Attempts made: " + stats.currentAttempt);
        } else {
            logCallback.accept("[ERROR] Password not found in search space");
            logCallback.accept("[STATS] Total attempts: " + stats.currentAttempt);
        }
    ...
    private boolean testPasswordsOfLength(String charset, int length, BruteForceConfig config, 
        ...
        for (long i = 0; i < passwordsAtThisLength && isRunning.get() && attemptCounter.get() < config.maxAttempts; i++) {
            ...
            if (currentAttempt % 100 == 0) {
                ...
                logCallback.accept("[SCAN] Attempt " + currentAttempt + ": " + currentPassword + 
    ...
    private boolean testPassword(String password, String target, Consumer<String> logCallback) {
        ...
        if (target.contains("ssh") || target.contains("22")) {
            ...
            logCallback.accept("[WARN] Protocol not supported for real testing: " + target);
            return false;
        }
    }
    ...
    private boolean testRealSSH(String password, String target, Consumer<String> logCallback) {
        try {
            ...
            logCallback.accept("[OK] SSH connection successful: " + password);
            return true;
    ...
    private boolean testRealHTTP(String password, String target, Consumer<String> logCallback) {
        try {
            ...
            if (response.statusCode() == 200) {
                logCallback.accept("[OK] HTTP auth successful: admin:" + password);
                return true;
            }
