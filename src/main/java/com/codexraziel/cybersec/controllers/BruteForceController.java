...
import com.codexraziel.cybersec.services.BruteForceGenerator;
import com.codexraziel.cybersec.services.BruteForceGenerator.BruteForceConfig;
import com.codexraziel.cybersec.services.BruteForceGenerator.BruteForceStats;
...
public class BruteForceController {
    ...
    public void calculateCombinations() {
        try {
            ...
            logArea.appendText("[STATS] CALCULATION RESULTS:\n");
            ...
            logArea.appendText("[ERROR] Calculation error: " + ex.getMessage() + "\n");
        }
    }
    ...
    private void startBruteForce() {
        ...
        logArea.appendText("[TARGET] Target: " + target + "\n");
        logArea.appendText("[CONFIG] Protocol: " + protocolCombo.getValue() + "\n");
        logArea.appendText("[USER] Username: " + usernameField.getText() + "\n");
        
        startBtn.setDisable(true);
        stopBtn.setDisable(false);
        attackProgress.setProgress(0);
        statusLabel.setText("Brute Force Running");
    ...
    private void stopBruteForce() {
        ...
        logArea.appendText("[STOP] STOPPING BRUTE FORCE ATTACK...\n");
        logArea.appendText("[OK] Attack stopped by user\n\n");
    ...
    private void onAttackComplete(BruteForceStats stats) {
        Platform.runLater(() -> {
            if (stats != null && stats.isFound()) {
                logArea.appendText("\n[CELEBRATION] SUCCESS! PASSWORD FOUND: " + stats.getFoundPassword() + "\n");
            } else {
                logArea.appendText("\n[ERROR] Password not found in search space\n");
            }
            
            if (stats != null) {
                logArea.appendText("[STATS] Total attempts: " + String.format("%,d", stats.getCurrentAttempt()) + "\n");
            }
    ...
    private Void onAttackError(Throwable throwable) {
        Platform.runLater(() -> {
            logArea.appendText("[ERROR] Error: " + throwable.getMessage() + "\n");
            resetUI();
        });
        return null;
    }
