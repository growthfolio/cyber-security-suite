...
public class ToolExecutor {
    ...
    private void simulateKeylogger(String platform, Consumer<String> outputCallback) {
        outputCallback.accept("[⚠️] Real keylogger not found in expected paths");
        outputCallback.accept("[💡] Build the keylogger first: cd tools/learning-c-so/linux && make");
        outputCallback.accept("[📝] Platform: " + platform);
        outputCallback.accept("[❌] Keylogger simulation disabled - use real tools only");
    }
    
    private void simulateBruteForce(String target, String protocol, int threads, Consumer<String> outputCallback) {
        outputCallback.accept("[⚠️] Real brute force tools not found");
        outputCallback.accept("[💡] Install hydra, medusa, or ncrack for real attacks");
        outputCallback.accept("[📝] Target: " + target + " (" + protocol + ")");
        outputCallback.accept("[❌] Brute force simulation disabled - use real tools only");
    }
}