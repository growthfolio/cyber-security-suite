...
public class WiFiScannerService {
    ...
    public CompletableFuture<ScanResult> scanNetworks(String networkInterface, Consumer<String> outputCallback) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                outputCallback.accept("[📡] Starting WiFi scan on interface: " + networkInterface);
                ...
                outputCallback.accept("[✅] Scan completed - " + networks.size() + " networks found");
                
            } catch (Exception e) {
                log.error("WiFi scan failed", e);
                errorMessage = e.getMessage();
                outputCallback.accept("[❌] Scan error: " + e.getMessage());
            }
                ...
                try (BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()))) {
                    ...
                    while ((line = reader.readLine()) != null) {
                        ...
                        if (network != null && !network.getSsid().isEmpty()) {
                            ...
                            outputCallback.accept("[📡] Found: " + network.getSsid());
                        }
                    }
                }
    ...
    private boolean scanWithIwlist(List<WiFiNetwork> networks, Consumer<String> outputCallback) {
        try {
            ...
            outputCallback.accept("[⚠️] iwlist parsing not fully implemented");
            return false;
    ...
    private void addTestNetworks(List<WiFiNetwork> networks, Consumer<String> outputCallback) {
        outputCallback.accept("[⚠️] No scanning tools available - showing test networks");
