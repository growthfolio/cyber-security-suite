...
import com.research.cybersec.services.WiFiScannerService;
import com.research.cybersec.services.WiFiPentestService;
import com.research.cybersec.services.ResourceCoordinator;
import com.research.cybersec.security.InputValidator;
import com.research.cybersec.security.PermissionManager;
import com.research.cybersec.security.AuditLogger;
import com.research.cybersec.services.ResultsExporter;
import com.research.cybersec.services.ConfigurationManager;
...
public class WiFiController {
    ...
    public void scanNetworks() {
        ...
        Platform.runLater(() -> {
            ...
            scanStatus.setText("📡 Scanning networks...");
            scanStatus.setStyle("-fx-text-fill: blue; -fx-font-weight: bold;");
            cancelBtn.setVisible(true);
            networksTable.getItems().clear();
        });
        
        wifiLogArea.appendText("[🔍] Starting WiFi network scan...\n");
        wifiLogArea.appendText("[📶] Interface: " + interfaceCombo.getValue() + "\n");
        ...
        currentScan = resourceCoordinator.executeWithResourceControl(operationId, networkInterface, () -> {
            ...
            Platform.runLater(() -> {
                ...
                if (scanResult.isSuccessful()) {
                    scanStatus.setText("✅ Scan completed - " + scanResult.getTotalNetworks() + " networks found");
                    ...
                    wifiLogArea.appendText("[✅] Scan completed - " + scanResult.getTotalNetworks() + " networks found\n");
                    wifiLogArea.appendText("[⚠️] Vulnerable networks: " + scanResult.getVulnerableNetworks() + "\n\n");
                    ...
                    scanStatus.setText("❌ Scan failed");
                    scanStatus.setStyle("-fx-text-fill: red; -fx-font-weight: bold;");
                    wifiLogArea.appendText("[❌] Scan failed: " + scanResult.getErrorMessage() + "\n\n");
            ...
            Platform.runLater(() -> {
                scanProgress.setVisible(false);
                cancelBtn.setVisible(false);
                scanStatus.setText("❌ Resource error");
                scanStatus.setStyle("-fx-text-fill: red; -fx-font-weight: bold;");
    ...
    private void startWiFiAttack() {
        ...
        wifiLogArea.appendText("[🎯] Target: " + ssid + "\n");
        wifiLogArea.appendText("[⚔️] Attack: " + attack + "\n");
        wifiLogArea.appendText("[📁] Wordlist: " + wordlist + "\n");
    ...
    private void stopWiFiAttack() {
        wifiLogArea.appendText("[⛔] Stopping WiFi attack...\n");
        ...
        wifiLogArea.appendText("[✅] Attack stopped by user\n\n");
    ...
    public void captureHandshake() {
        wifiLogArea.appendText("[🤝] Capturing WPA handshake...\n");
        wifiLogArea.appendText("[⚙️] Monitoring for authentication packets...\n");
        wifiLogArea.appendText("[✅] Handshake captured successfully\n\n");
    }
    ...
    public void cancelScan() {
        if (currentScan != null && !currentScan.isDone()) {
            ...
            Platform.runLater(() -> {
                ...
                wifiLogArea.appendText("[⛔] Network scan cancelled by user\n\n");
            });
        }
    }
    ...
    public void saveResults() {
        ...
        wifiLogArea.appendText("[💾] Saving scan results...\n");
        ...
        if (result.isSuccess()) {
            wifiLogArea.appendText("[✅] Results saved: " + result.getFilePath() + "\n");
            wifiLogArea.appendText("[📊] Records exported: " + result.getRecordCount() + "\n\n");
            showInfo("Save Successful", "Results saved to: " + result.getFilePath());
        } else {
            wifiLogArea.appendText("[❌] Save failed: " + result.getMessage() + "\n\n");
    ...
    public void exportResults() {
        ...
        wifiLogArea.appendText("[📤] Exporting results...\n");
    ...
    public void analyzeTarget() {
        ...
        wifiLogArea.appendText("[🔍] Analyzing target: " + ssid + "\n");
    ...
    private void showExportDialog() {
        ...
        alert.showAndWait().ifPresent(response -> {
            ...
            if (result != null) {
                if (result.isSuccess()) {
                    wifiLogArea.appendText("[✅] Export completed: " + result.getFilePath() + "\n");
                    wifiLogArea.appendText("[📊] Records exported: " + result.getRecordCount() + "\n\n");
                    showInfo("Export Successful", "Results exported to: " + result.getFilePath());
                } else {
                    wifiLogArea.appendText("[❌] Export failed: " + result.getMessage() + "\n\n");
    ...
    public void saveProfile() {
        ...
        dialog.showAndWait().ifPresent(profileName -> {
            ...
            if (result.isSuccess()) {
                wifiLogArea.appendText("[💾] Profile saved: " + profileName + "\n\n");
                showInfo("Profile Saved", "Profile '" + profileName + "' saved successfully.");
            } else {
                wifiLogArea.appendText("[❌] Profile save failed: " + result.getMessage() + "\n\n");
                showError("Save Failed", result.getMessage());
            }
        });
    }
    ...
    public void loadProfile() {
        ...
        dialog.showAndWait().ifPresent(selectedProfile -> {
            ...
            if (result.isSuccess()) {
                ...
                wifiLogArea.appendText("[💾] Profile loaded: " + selectedProfile + "\n\n");
                showInfo("Profile Loaded", "Profile '" + selectedProfile + "' loaded successfully.");
            } else {
                wifiLogArea.appendText("[❌] Profile load failed: " + result.getMessage() + "\n\n");
